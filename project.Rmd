...
Our approach is general linear regression with R. We firstly cleaned and categorized data in a property way that reduces the fraction of misleading results. Then, from pair plot, we concluded a pre-fitting model and, using automated model selection techniques, we obtained another three models. We then used manual model selection methods including PRESS, $R^2$, AIC and narrowed the number of our candidate models down to two. Then we further diagonsed the two models by checking their linear assumptions, leverage, and outliers and also by runnning cross validations. Finally, we drew conclusions on the best model we obtained and hence found a reasonably good relation between between the weight of healthy male single-fetus baby and some possible factors.

Based on the preferred model, we propose that ...
...
# Categorize m/feth
#meth.categories = c(rep('Caucasian', 6), 'Mexican', 
...
# Categorize m/fed
#med.categories = c('elementary school', 'middle school', 'high school', 
#                   'high school + trade school', 'high school + some college', 
#                   'college graduate', 'trade school', 'high school unclear')
...
# Categorize marital
#marital.categories = c('married', 'legally separated', 
...
#                    'during pregnancy', 'less than a year', '1-2 years', 
#                   '2-3 years', '3-4 years', '5-9 years', 
...
# Categorize number
#number.categories = c('never smoked', '1-4', '5-9', '10-14', '15-19',
#                      '20-29', '30-39', '40-60', 'more than 60', 
...
summary(MPrefit)
```
We got a pre-fitting model $MPrefit$ by analyzing pair plots of continuous variable. We noticed that there is a strong linear relation between $mage$ and $fage$. By analyzing variance inflaction factor, fage has more correlated relation with other variables. Hence in the pre-fitting model, we decided to remove the $fage$.
...
```
\newline
We found that observation 240 had the largest residual in $Mstep$ and so it is the outlier of $MStep$. For this observation, 
