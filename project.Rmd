...
```
Here is a list of important factors extracted from the p-value column of the anova table generate by R. The list with the smallest p-value includes gestation, ethnicity of mother, mother height, mother weight, father height, father weight, when mother quit smoke before pregnacy and gestation interacts with income.  
...
```
* From a graphical point of view, we can see that the residuals plot for stepwise model satisfies mean of zero and the model follows normality but violates constant varaince. The model is selected by automated model selection which depended on the choice of MStart and Mfull. This might cause the deficiencies of the final model.



#Conclusion
Overall, the stepwise selection model is chosen to be the best fitting regression model after several rounds of comparsions. Our final model has been narrowed down to 55 covariates including intercept with gestation, mother's race, mother's height, mother's weight, father's wieght, when mother quit smoke before pregnacy and gestation interacts with income being the most significants parameters in the model.
