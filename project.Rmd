...
We got 4 different models, $MFwd$, $MBack$, $MStep$ and $MPrefit$, by performing forward selection, backward elimination and stepwise selection on full model respectively. From the above table generated by R, we can see that $MBack$ has the smallest AIC Press Statistic  and has the largest $R^2$ compared to the other two models. By comparing $MStep$ and $MFwd$, $MStep$ seems to have a better $R^2$ and a better AIC than $MFwd$, but $MFwd$ only has a better Press Statistic than $MStep$. Therefore, $Mback \cap MStep \in$ our canadiate set.
